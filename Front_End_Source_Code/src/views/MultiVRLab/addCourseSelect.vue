<template>
  <div  style="width: 80%;text-align: center">
    <h2 style="margin-bottom: 50px">录 入 选 课 信 息</h2>
    <el-form :inline="true" :model="form" :rules="rules" ref="ruleForm" class="demo-form-inline" label-width="130px">
      ...
      <el-form-item label="任课教师" prop="teacher_name">
        <el-input v-model="form.teacher_name" placeholder="请输入任课教师姓名"></el-input>
      </el-form-item>
      <el-form-item label="任课教师学工号" prop="teacher_number">
        <el-input v-model="form.teacher_number" placeholder="请输入任课教师学工号"></el-input>
      </el-form-item>
    </el-form>
  ...
  data(){
    ...
    return{
      ...
      rules: {
        ...
        teacher_number: [
          { required: true, message: '请输入教师学工号', trigger: 'blur' }
        ],
      }
    }
  },
  methods:{
    save(){
      this.$refs["ruleForm"].validate((valid)=>{
        if(valid){
          request.post('/courseSelect/save',this.form).then(res =>{
            if(res.code === '200'){
              this.$notify({
                title: "操作提示",
                message: "录入成功",
